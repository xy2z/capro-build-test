...
namespace _PhpScoper5e9ecd738c28\Statix\Server;

use _PhpScoper5e9ecd738c28\Dotenv\Dotenv;
use Exception;
use _PhpScoper5e9ecd738c28\Symfony\Component\Process\PhpExecutableFinder;
...
class Server
{
    ...
    public function __construct(array $configuration = [])
    {
        ...
        $this->configuration = \array_merge($defaults = ['host' => 'localhost', 'port' => 8000, 'root' => \getcwd(), 'php' => null, 'router' => null, 'withEnvVars' => [], 'withoutEnvVars' => []], $configuration);
    ...
    {
        $this->envVarsToPass = \array_filter($this->envVarsToPass, $callback, \ARRAY_FILTER_USE_BOTH);
        return $this;
    }
    ...
    {
        $this->envVarsToPass = \array_merge($this->envVarsToPass, $vars);
        return $this;
    }
    /**
    ...
    {
        $this->envVarsToPass = \array_filter($this->envVarsToPass, function ($key) use($vars) {
    ...
    {
        $command = [$this->findExecutable(), '-S', $this->configuration['host'] . ':' . $this->configuration['port'], '-t', $this->configuration['root']];
    ...
    {
        $process = new Process($this->buildServeCommand(), null, $this->buildPassingEnvVarArray(), null, null);
    ...
    public function stop() : array|null
    {
        ...
        $this->process->stop(1);
        $this->running = \false;
        return [$this->process->getExitCode(), $this->process->getExitCodeText()];
    }
    /**
