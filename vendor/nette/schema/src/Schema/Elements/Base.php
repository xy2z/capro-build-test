...
namespace _PhpScoper5e9ecd738c28\Nette\Schema\Elements;

use _PhpScoper5e9ecd738c28\Nette;
use _PhpScoper5e9ecd738c28\Nette\Schema\Context;
/**
 * @internal
 */
trait Base
{
    ...
    private function doValidate($value, string $expected, Context $context) : bool
    {
        if (!Nette\Utils\Validators::is($value, $expected)) {
            ...
            $context->addError('The %label% %path% expects to be %expected%, %value% given.', Nette\Schema\Message::TypeMismatch, ['value' => $value, 'expected' => $expected]);
            return \false;
        }
        return \true;
    }
    private function doValidateRange($value, array $range, Context $context, string $types = '') : bool
    {
        if (\is_array($value) || \is_string($value)) {
            ...
            if (!self::isInRange($length, $range)) {
                $context->addError("The length of %label% %path% expects to be in range %expected%, %length% {$label} given.", Nette\Schema\Message::LengthOutOfRange, ['value' => $value, 'length' => $length, 'expected' => \implode('..', $range)]);
                return \false;
            }
        } elseif ((\is_int($value) || \is_float($value)) && !self::isInRange($value, $range)) {
            $context->addError('The %label% %path% expects to be in range %expected%, %value% given.', Nette\Schema\Message::ValueOutOfRange, ['value' => $value, 'expected' => \implode('..', $range)]);
            return \false;
        }
        return \true;
    }
