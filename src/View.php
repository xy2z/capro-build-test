...
use Exception;
use _PhpScoper5e9ecd738c28\Spatie\YamlFrontMatter\YamlFrontMatter;
use _PhpScoper5e9ecd738c28\Jenssegers\Blade\Blade;
use xy2z\Capro\PublicView;
use _PhpScoper5e9ecd738c28\Spatie\YamlFrontMatter\Document;
use Throwable;
class View
{
    ...
    // needed for cli messages (errors).
    protected string $basename;
    protected string $label;
    protected string $save_path;
    // full path to where the build view file (html) will be saved.
    ...
    {
        $data = \array_merge($this->view_data, $this->yaml_front_matter->matter(), ['self' => $this->get_public_view()]);
        if (isset($key)) {
    ...
    public function build() : bool
    {
        ...
        if ($saved === \false) {
            throw new Exception('Could not build view.');
            // TODO: If this is in CommandServe, just retry in a few microseconds...
        }
        // Build (make view)
        $build_content = null;
        try {
            $make = self::$blade->make('__tmp', $this->get_view_data());
            $build_content = $make->render();
            // Do this here so it can throw exceptions.
        } catch (Throwable $e) {
    ...
    protected static function load_blade() : void
    {
        ...
        \class_alias('xy2z\\Capro\\Config', '_PhpScoper5e9ecd738c28\\Config');
