...
class Collector
{
    ...
    public function first() : ?\xy2z\Capro\PublicView
    {
        return $this->data[\array_key_first($this->data)] ?? null;
    }
    public function last() : ?\xy2z\Capro\PublicView
    {
    ...
    public function orderBy(string $sort_key, bool $case_insensitive = \true) : self
    {
    ...
    public function orderByDesc(string $sort_key, bool $case_insensitive = \true) : self
    {
    ...
    public function exclude(\xy2z\Capro\PublicView|array $exclude_views) : self
    {
        if (!\is_array($exclude_views)) {
            $exclude_views = [$exclude_views];
        }
        foreach ($exclude_views as $view) {
            if (!$view instanceof \xy2z\Capro\PublicView) {
    ...
    public function debug() : void
    {
        $debug_data = [];
        foreach ($this->data as $key => $view) {
            $debug_data[$key] = ['path' => $view->path, 'relative_path' => $view->relative_path, 'dir' => $view->dir, 'basename' => $view->basename, 'href' => $view->href, 'view_data' => $view->get_all_view_data()];
        }
